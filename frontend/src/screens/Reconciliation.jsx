      ...
      <div className="recon-grid">
        <div className="card">
          <div className="card-h">
            <div><h3>Cobertura por fuente</h3><p>Documentos importados este mes</p></div>
        ...
        <div className="card">
          <div className="card-h">
            <div><h3>Decisiones pendientes</h3><p>5 elementos requieren tu revisión</p></div>
            <span className="chip warn-chip"><Icon name="review" size={10} /> Pendientes</span>
          </div>
          <DecisionRow kind="duplicate" title="Posible duplicado"          desc="Mercado Central — $84.50 — 24 Abr"       sub="Coincide con cargo manual del 24 Abr · 14:32"              actions={['Es el mismo', 'Son distintos']} />
          <DecisionRow kind="match"     title="Sugerencia de match"        desc="Transferencia $500 → Ahorros"            sub="Vincular con obligación 'Meta de ahorro mensual'"           actions={['Vincular', 'Ignorar']} />
          <DecisionRow kind="card"      title="Pago de tarjeta sin clasificar" desc="$640.00 a Tarjeta Oro"             sub="¿Es liquidación de saldo o gasto del mes?"                  actions={['Liquidación', 'Gasto']} />
          <DecisionRow kind="missing"   title="Movimiento esperado faltante"   desc="Hipoteca BBVA — $1,420 — esperado 28 Abr" sub="Programado, aún no aparece en estado de cuenta"    actions={['Marcar pagado', 'Esperar']} />
        </div>

        <div className="card audit-card">
          <div className="card-h"><div><h3>Bitácora reciente</h3><p>Últimos cambios y decisiones</p></div></div>
        ...
        <div className="card carry-card">
          <div className="card-h">
            <div><h3>Preparación · Mayo 2026</h3><p>Vista previa del próximo mes</p></div>
...
function SourceRow({ name, type, rows, pct, status, date }) {
  return (
...
function DecisionRow({ kind, title, desc, sub, actions }) {
...
function AuditRow({ time, who, action, target }) {
  return (
    <div className="audit-row">
      <div className="audit-avatar">{who}</div>
      <div style={{ flex: 1 }}>
        <div className="audit-line"><span className="audit-action">{action}</span> <span className="audit-target">{target}</span></div>
...
function CarryStat({ label, value, tone }) {
  return (
