// Dashboard — hero "true available balance" + cash flow + KPIs + ghost charges
const Dashboard = () => {
  // Cash flow forecast — past 3 + future 3 months
  const months = [
    { m: "Ene", inc: 5800, exp: 3200 },
    { m: "Feb", inc: 6100, exp: 3650 },
    { m: "Mar", inc: 5950, exp: 3400 },
    { m: "Abr", inc: 6420, exp: 3847, current: true },
    { m: "May", inc: 6500, exp: 3950, forecast: true },
    { m: "Jun", inc: 6500, exp: 4100, forecast: true },
  ];
  const max = 7000;
    ...
    <>
      ...
      <div className="card hero-balance">
        <div className="hero-left">
          ...
          <div className="num num-hero">
            $18,433<span className="cents">.<span>40</span></span>
          </div>
        ...
        <div className="hero-right">
          <div className="hero-pill">
            <div className="hero-pill-h">
              <Icon name="income" size={13} style={{ color: "var(--pos)" }}/>
              <span>Ingresos</span>
            </div>
          ...
          <div className="hero-pill">
            <div className="hero-pill-h">
              <Icon name="expense" size={13} style={{ color: "var(--neg)" }}/>
              <span>Gastos</span>
            </div>
        ...
        <div className="card">
          <div className="card-h">
            <div>
              ...
              <p>Ingresos vs Gastos · Pronóstico 3 meses</p>
            </div>
            <div className="tabs">
          ...
          <div className="cashflow-legend">
            <span className="row" style={{ gap: 6 }}>
              <span className="legend-dot" style={{ background: "var(--accent)" }}/> Ingresos
            </span>
            <span className="row" style={{ gap: 6 }}>
              <span className="legend-dot" style={{ background: "var(--accent-3)" }}/> Gastos
            </span>
            <span className="row" style={{ gap: 6, marginLeft: "auto" }}>
              <span className="legend-dot dashed"/> Pronóstico
            </span>
          </div>
            ...
            {months.map((mo, i) => (
              <div key={i} className={`bar-col ${mo.current ? "current" : ""}`}>
                <div className="bar-stack">
                  <div
                    className={`bar inc ${mo.forecast ? "forecast" : ""}`}
                    style={{ height: `${(mo.inc / max) * 100}%` }}
                  />
                  <div
                    className={`bar exp ${mo.forecast ? "forecast" : ""}`}
                    style={{ height: `${(mo.exp / max) * 100}%` }}
                  />
                </div>
      ...
      {/* Insights row: ghost charges + upcoming */}
      <div className="grid-2">
        <div className="card insight ghost-card">
          ...
          <button className="btn ghost sm" style={{ marginTop: 12 }}>
            Revisar todas <Icon name="chevron-right" size={12} />
          </button>
        </div>

        <div className="card insight">
          <div className="insight-h">
            <div className="insight-icon" style={{ background: "rgba(99,102,241,0.14)", color: "var(--accent)" }}>
...
const KPI = ({ icon, label, value, delta, tone = "pos", tint }) => (
...
const Sparkline = ({ points, color, small }) => (
...
const GhostRow = ({ logo, color, name, amount, since }) => (
...
const UpRow = ({ date, name, sub, amount, tag, tagKind }) => (
